...
/**
 * Validation GPS sur le lieu de l'énigme (prise de position dynamique en jeu uniquement).
 */
const validerPosition = () => {
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            const cibleLat = Number(props.enigme.place.latitude);
            const cibleLng = Number(props.enigme.place.longitude);
            const rayon = Number(props.enigme.place.rayon_validation ?? 30);

            const distance = distanceMetres(lat, lng, cibleLat, cibleLng);

            if (distance <= rayon) {
                alert(`Bonne position ! Vous êtes à ${Math.round(distance)} m du lieu.`);
            } else {
                alert(`Vous êtes à ${Math.round(distance)} m du lieu (rayon : ${rayon} m). Rapprochez-vous.`);
            }
        },
        () => {
